function [r,v,theta_E,a_prop_E,e_prop_E,Omega_prop_E,omega__E,i_prop_E,r_E] = state_vector_JC(planet_OE,jul_cent)
%=======================================
...
e_E         = planet_OE(1,2);            % #
e_dot_E     = planet_OE(2,2);            % 1 / Cy

i_E         = planet_OE(1,3);            % Grad
i_dot_E     = planet_OE(2,3);            % Grad / Cy
...
i_prop_E     = deg2rad(Propagation(i_E,i_dot_E,jul_cent));                    % Inclination 0<=i<pi (results in radians).
...
omega__E = mod(argp(omega_prop_E,Omega_prop_E), 2*pi);  % Result in Rads
...
R_E = rotational_matrix(Omega_prop_E, omega__E, i_prop_E);
